/*
 AFNotificationBadge.m refactored from CustomBadge.m
...
#import "AFNotificationBadge.h"

@interface AFNotificationBadge()
- (void) drawRoundedRectWithContext:(CGContextRef)context withRect:(CGRect)rect;
- (void) drawFrameWithContext:(CGContextRef)context withRect:(CGRect)rect;
@end

@implementation AFNotificationBadge
...
@synthesize badgeBaseSize;

// I recommend to use the allocator customBadgeWithString
- (id) initWithString:(NSString *)badgeString withScale:(CGFloat)scale withShining:(BOOL)shining withRectFrame:(CGRect)rect
{
	self = [super initWithFrame:rect]; // default was (0,0,25,25)
	if(self!=nil) {
        if ([self respondsToSelector:@selector(contentScaleFactor)]){
            self.contentScaleFactor = [[UIScreen mainScreen] scale];
        }
        self.badgeBaseSize = rect.size;
...
{
    CGSize defaultBaseSize = CGSizeMake(25, 25);
    
	self = [super initWithFrame:CGRectMake(0, 0, defaultBaseSize.width, defaultBaseSize.height)];
	if(self!=nil) {
        self.badgeBaseSize = defaultBaseSize;
		self.contentScaleFactor = [[UIScreen mainScreen] scale];
		self.backgroundColor = [UIColor clearColor];
		self.badgeText = badgeString;
...
- (void) autoBadgeSizeWithString:(NSString *)badgeString
{
	...
	if ([badgeString length]>=2) {
		...
		rectWidth = origWidth + (stringSize.width + flexSpace);
        rectHeight = origHeight;
		retValue = CGSizeMake(rectWidth*badgeScaleFactor, rectHeight*badgeScaleFactor);
	} else {
		retValue = CGSizeMake(origWidth*badgeScaleFactor, origWidth*badgeScaleFactor);
	}
...
+ (AFNotificationBadge*) customBadgeWithString:(NSString *)badgeString withRectFrame:(CGRect)rect
{
    return [[[self alloc] initWithString:badgeString withScale:1.0 withShining:YES withRectFrame:rect] autorelease];
} // customBadgeWithString

// Creates a Badge with a given Text 
+ (AFNotificationBadge*) customBadgeWithString:(NSString *)badgeString
{
	return [[[self alloc] initWithString:badgeString withScale:1.0 withShining:YES withRectFrame:CGRectMake(0, 0, 25, 25)] autorelease];
}

// Creates a Badge with a given Text, Text Color, Inset Color, Frame (YES/NO) and Frame Color 
+ (AFNotificationBadge*) customBadgeWithString:(NSString *)badgeString withStringColor:(UIColor*)stringColor withInsetColor:(UIColor*)insetColor withBadgeFrame:(BOOL)badgeFrameYesNo withBadgeFrameColor:(UIColor*)frameColor withScale:(CGFloat)scale withShining:(BOOL)shining
{
